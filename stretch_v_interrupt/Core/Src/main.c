...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(CG_GPIO_Port, CG_Pin, GPIO_PIN_RESET);
...
{
        if (htim->Instance == TIM4 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) // 채널 1(직접모드) PWM ?��?�� ?��?���?????????? ?��?��?���??????????
        {
